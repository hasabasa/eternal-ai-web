...
const ClientCalc = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-tr from-brand-purple/20 to-brand-orange/20 px-4 py-10 animate-fade-in">
      <div className="max-w-md w-full bg-white/95 rounded-2xl shadow-xl border border-muted p-8 flex flex-col items-center">
        ...
        <div className="w-full flex flex-col gap-3">
          ...
          <label className="text-md font-medium">Процент (%)
            <input
              type="number"
              className="w-full mt-1 px-3 py-2 rounded-lg border border-muted bg-muted/40 focus:outline-none focus:ring-2 focus:ring-brand-orange transition"
              value={percent}
          ...
          <button
            onClick={handleCalc}
            className="mt-4 px-6 py-2 font-medium rounded-lg bg-primary text-primary-foreground hover:bg-brand-orange/90 transition"
          >
            Рассчитать
          </button>
          {result !== null && (
        ...
        <Link to="/" className="mt-8 px-5 py-2 font-medium rounded-lg bg-brand-darkBlue text-white hover:bg-brand-orange transition">
          На главную
        </Link>
      </div>
    </div>
