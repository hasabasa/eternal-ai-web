...
const steps = [
  ...
  {
    icon: <Settings className="w-12 h-12 text-brand-orange" />,
    title: "Настройка ИИ",
    desc: "Создаём ассистента под потребности",
    duration: "3-5 дней"
  },
  {
    icon: <Rocket className="w-12 h-12 text-brand-purple" />,
    title: "Запуск",
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
      {steps.map((step, i) => (
        <div key={i} className="relative">
          ...
          <div className="bg-white/80 rounded-2xl p-8 text-center hover:bg-white/95 transition-all shadow-lg border border-white/40 backdrop-blur-sm">
            ...
            <div className="text-lg lg:text-xl bg-brand-purple/10 text-brand-purple px-4 py-3 rounded-full">
              {step.duration}
            </div>
          </div>
        </div>
